...
163FORL%=I%TOE%:E=TAN(LN(C)):NEXT:T%=TIME:PROCr
164UNTILT>L:PROCp
165:
170PRINT"String manipulation ";:B%=LENU$DIV2:S%=B%DIV2:Q%=S%+1:C%=2
171REPEATPROCi
172REPEATUNTILTIME=T%
173FORL%=I%TOE%:C%=LEN(RIGHT$(MID$(U$,B%,Q%)+RIGHT$(U$,S%)+MID$(U$,B%,C%)+LEFT$(U$,S%)+MID$(U$,S%,Q%),LENU$)):C%=LEN(RIGHT$(MID$(U$,B%,Q%)+RIGHT$(U$,S%)+MID$(U$,B%,C%)+LEFT$(U$,S%)+MID$(U$,S%,Q%),LENU$)):NEXT:T%=TIME:PROCr
174UNTILT>L:PROCp
175:
...
240PRINTSPC(16)"+/-   ";:IFL>100PRINT"0."STRING$((LOGL)-3,"0")"2%"ELSEPRINT200/L;"%"
...
430T%=2:TIME=0:ENDPROC
440:
450DEFPROCr:T=T%-1.5:B(N%)=L%/T:ENDPROC
460:
470DEFPROCp:IFN%<10THENC=L%:READE:ELSEE=1
480IFZ%AND&80PRINTC,T,2*T/C" ";
490T=(M*C*E/T)+0.5/L
500IFT<L PRINTMID$(STR$(T),1,LOG(L)+2);ELSEPRINTINT(T);
