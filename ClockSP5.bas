...
55RETURN
57:
60DIMA(10),B(10),U%5:FORC=0TO9:READA(C):B(C)=4/A(C):NEXT
...
142FORF%=I%TOE%:FORC=X TOE STEPS:NEXT:NEXT:T%=TIME:PROCr
143UNTILT>L:PROCp
145:
...
210FORA%=0TO9:C=C+A(A%)*B(A%):NEXT:T=V%
220PROCp
...
290:
300DEFPROCs
305IFINKEY(-256)>1ORINKEY(-256)<&F0 Z%=Z%+&2000
310H%=Z%AND&F000:IFH%=0THENH%=FNB
320IFH%=4RESTORE800
330IFH%=5RESTORE1000
...
460:
470DEFPROCp:IFN%<10THENC=F%
475IFZ%AND&80PRINTC,T,2*T/C" ";
480T=(M*C*A(N%)/T)+0.5/L
